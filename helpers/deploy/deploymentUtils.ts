...
// Networks not listed here (hardhat included) fall back to the deployer.
const guardianByNetwork: Partial<Record<SUPPORTED_NETWORKS, string>> = {
...
export const guardian = async (network: SUPPORTED_NETWORKS): Promise<string> => {
  ...
  return guardianByNetwork[network] ?? deployer;
};
