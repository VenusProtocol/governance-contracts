...
import "./MultiTokenGovernorBravoInterface.sol";

/**
 * @title MultiTokenGovernorBravoDelegate
 * @author Venus
 */
contract MultiTokenGovernorBravoDelegate is MultiTokenGovernorBravoDelegateStorageV2, MultiTokenGovernorBravoEvents {
    /// @notice The name of this contract
    string public constant name = "Venus Multi Token Governor Bravo";

    /// @notice The minimum setable proposal threshold
    uint public constant MIN_PROPOSAL_THRESHOLD = 150000e18; // 150,000

    /// @notice The maximum setable proposal threshold
    uint public constant MAX_PROPOSAL_THRESHOLD = 300000e18; //300,000
    ...
    uint public constant quorumVotes = 600000e18; // 600,000 = 2%
    ...
    function initialize(
        ...
        require(address(proposalTimelocks[0]) == address(0), "cannot initialize twice");
        require(msg.sender == admin, "admin only");
        require(vaultAggregator_ != address(0), "invalid VaultAggregator address");
        require(guardian_ != address(0), "invalid guardian");
        require(
            timelocks.length == uint8(ProposalType.CRITICAL) + 1,
            "number of timelocks should match number of governance routes"
        );
        require(
        ...
        vaultAggregator = VaultAggregatorInterface(vaultAggregator_);
        ...
        for (uint256 i; i < arrLength; ++i) {
            ...
            require(address(timelocks[i]) != address(0), "invalid timelock address");
    ...
    function propose(
        ...
        require(initialProposalId != 0, "Governor Bravo not active");
        ...
        require(
            vaultAggregator.getPriorVotes(msg.sender, sub256(block.number, 1), proposalTokenWeight) >=
    ...
    function cancel(uint proposalId) external {
        ...
        require(
            msg.sender == guardian ||
                ...
                vaultAggregator.getPriorVotes(proposal.proposer, sub256(block.number, 1), proposal.weights) <
