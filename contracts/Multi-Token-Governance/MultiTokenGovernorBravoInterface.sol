...
/**
 * @title MultiTokenGovernorBravoDelegatorStorage
 * @author Venus
 * @notice Storage layout of the `GovernorBravoDelegator` contract
 */
contract MultiTokenGovernorBravoDelegatorStorage {
 ...
 */
contract MultiTokenGovernorBravoDelegateStorageV1 is MultiTokenGovernorBravoDelegatorStorage {
    ...
    VaultAggregatorInterface public vaultAggregator;
...
interface VaultAggregatorInterface {
    function getPriorVotes(address account, uint blockNumber, uint8[] calldata weights) external view returns (uint96);
}
