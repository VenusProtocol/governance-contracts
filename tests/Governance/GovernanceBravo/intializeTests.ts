...
describe("Governor Bravo Initializing Test", () => {
  ...
  describe("Check setter functions", () => {
    ...
    it("should revert if not called by admin", async () => {
      await expect(governorBravoDelegate.connect(customer)._setGuardian(accounts[0].getAddress())).to.be.rejectedWith('Unauthorized("0x70997970C51812dc3A010C7d01b50e0d17dc79C8", "0xB0D4afd8879eD9F52b28595d31B441D079B2Ca07", "_setGuardian(address)")');
    });

    it("should allow setting guardian with access permission", async () => {
      await expect(await accessControlManager.connect(root).giveCallPermission(
        governorBravoDelegate.address,
        "_setGuardian(address)",
        await root.getAddress(),
      )).to.emit(accessControlManager, "PermissionGranted");

      await expect(governorBravoDelegate.connect(root)._setGuardian(await customer.getAddress())).to.emit(governorBravoDelegate, "NewGuardian")
