...
describe("MultiTokenGovernance", async () => {
  ...
  const GovernanceFixture = async () => {
    ...
    const MultiTokenGovernanceFactory = await ethers.getContractFactory("MultiTokenGovernance");
    governorBravoDelegate = await upgrades.deployProxy(MultiTokenGovernanceFactory, [
      vaultAggregator.address,
      [
        [1, 12 * 60 * 5, convertToUnit("150000", 18)],
        [1, 12 * 60 * 4, convertToUnit("200000", 18)],
        [1, 12 * 60 * 3, convertToUnit("250000", 18)],
      ],
    ...
    tx = await accessControl.giveCallPermission(vaultAggregator.address, "setMaxVaults(uint8)", deployer.address);

    await expect(vaultAggregator.updateVault(tokenVault2.address, true)).to.emit(vaultAggregator, "UpdateVault");
    await expect(vaultAggregator.updateDefaultWeights([100, 0])).to.emit(vaultAggregator, "DefaultWeightsUpdated");
  ...
  describe("VaultAggregator", async () => {
    let tokenVault3: TokenVault;
    beforeEach(async () => {
      const tokenVaultFactory = await ethers.getContractFactory("TokenVault");
      tokenVault3 = await upgrades.deployProxy(tokenVaultFactory, [accessControl.address], {
        constructorArgs: [token1.address, false, 10512000],
        initializer: "initialize",
        unsafeAllow: ["constructor"],
      });
    });
    it("Revert if attempt to add vaults greater than max allowed in vaultAggregator", async () => {
      await expect(vaultAggregator.updateVault(tokenVault3.address, true)).to.be.revertedWithCustomError(
        vaultAggregator,
        "VaultLimitExceed",
      );
    });
    it("Revert if attempt to add zero address in vaults", async () => {
      await expect(vaultAggregator.updateVault(ethers.constants.AddressZero, true)).to.be.revertedWithCustomError(
        vaultAggregator,
        "ZeroAddressNotAllowed",
      );
    });
    it("Reverts if address not found on deletion", async () => {
      await expect(vaultAggregator.updateVault(tokenVault3.address, false)).to.be.revertedWithCustomError(
        vaultAggregator,
        "VaultNotFound",
      );
    });
    it("Delete and add Vault successfully", async () => {
      await expect(vaultAggregator.updateVault(tokenVault1.address, false)).to.emit(vaultAggregator, "UpdateVault");
      await expect(vaultAggregator.updateVault(tokenVault1.address, true)).to.emit(vaultAggregator, "UpdateVault");
    });
    it("Increase max vault limit successfully", async () => {
      await expect(vaultAggregator.setMaxVaults(8)).to.emit(vaultAggregator, "SetMaxVaults");
    });
    it("Revert if zero max vault limit provided", async () => {
      await expect(vaultAggregator.setMaxVaults(0)).to.be.revertedWithCustomError(
        vaultAggregator,
        "ZeroValueNotAllowed",
      );
    });
    it("Revert if there's a length mismatch when setting default weights", async () => {
      await expect(vaultAggregator.updateDefaultWeights([80])).to.be.revertedWithCustomError(
        vaultAggregator,
        "InvalidLength",
      );
    });
    it("Successfully update default weights", async () => {
      await expect(vaultAggregator.updateDefaultWeights([80, 20])).to.emit(vaultAggregator, "DefaultWeightsUpdated");
    });
    it("Revert if permission is not granted", async () => {
      await expect(vaultAggregator.connect(signer1).updateDefaultWeights([30, 100])).to.be.revertedWithCustomError(
        vaultAggregator,
        "Unauthorized",
      );
      await expect(vaultAggregator.connect(signer1).setMaxVaults(20)).to.be.revertedWithCustomError(
        vaultAggregator,
        "Unauthorized",
      );
      await expect(
        vaultAggregator.connect(signer1).updateVault(tokenVault1.address, true),
      ).to.be.revertedWithCustomError(vaultAggregator, "Unauthorized");
    });
  });
  describe("MultiTokenGovernance", async () => {
    ...
    it("No votes will count if given weights and default weights are zero", async () => {
      await expect(vaultAggregator.updateDefaultWeights([0, 0])).to.emit(vaultAggregator, "DefaultWeightsUpdated");
      await expect(
        governorBravoDelegate.propose(targets, values, signatures, callDatas, "", ProposalType.NORMAL, []),
      ).to.emit(governorBravoDelegate, "ProposalCreated");
      const proposalId = await governorBravoDelegate.latestProposalIds(deployer.address);
