...
// Returns null (after warning) when there is no snapshot to relabel for `network`.
export function relabelNetwork(network: Network, opts: { baseDir?: string } = {}): RelabelResult | null {
  const { baseDir } = opts;
  ...
  const newFile = stateToFile(
    state,
    {
      ...
      // relabel makes no chain calls, so it cannot change the verification status — carry the
